...
@section('content')
    <div class="max-w-2xl">
        <div class="flex items-center mb-6">
            ...
            <h2 class="text-2xl font-bold text-gray-800">動画を編集</h2>
        </div>
            ...
            <form action="{{ route('admin.videos.update', $video) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-6">
                    ...
                    <textarea name="description" id="description" rows="3"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">{{ old('description', $video->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <select name="category" id="category" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                        <option value="short" {{ old('category', $video->category) === 'short' ? 'selected' : '' }}>ショート動画</option>
                        <option value="horizontal" {{ old('category', $video->category) === 'horizontal' ? 'selected' : '' }}>横動画</option>
                    </select>
                    @error('category')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-6">
                    ...
                    @if($video->thumbnail)
                        <div class="mb-2">
                            <img src="{{ Storage::url($video->thumbnail) }}" alt="現在のサムネイル" class="w-32 h-auto rounded">
                        </div>
                    @endif
